...
export const setupBot = () => {
    // 1. КОМАНДА /START С ЗАПРОСОМ ТЕЛЕФОНА И УСТАНОВКОЙ КНОПКИ МЕНЮ
    bot.command('start', async (ctx) => {
        const userId = ctx.from.id;
        // Устанавливаем главную кнопку "Меню", которая будет открывать наше приложение
        await ctx.setChatMenuButton({
            type: 'web_app',
            text: 'Открыть Дневник', // Текст на кнопке
            web_app: { url: `http://185.5.207.57:3000` } // ЗАМЕНИ YOUR_SERVER_IP НА СВОЙ IP!
        });
        ...
        await supabase.from('users').upsert({
            ...
            // Убираем дефолтную цель, т.к. теперь есть анкета
        }, { onConflict: 'telegram_id' });
        // Отправляем приветствие с КНОПКОЙ запроса телефона
        ctx.reply(`👋 Привет, ${ctx.from.first_name}!\nСлева от поля ввода текста появилась кнопка "Открыть Дневник". Нажми на нее, чтобы заполнить анкету.\n\nА для полной авторизации нажми кнопку ниже, чтобы отправить свой номер телефона.`, Markup.keyboard([
    ...
    bot.on(message('contact'), async (ctx) => {
        ...
        if (error) {
            ...
            ctx.reply("✅ Авторизация успешна! Номер сохранен.\nТеперь ты можешь пользоваться всеми функциями: скидывать фото еды, голосовые или текст.", Markup.removeKeyboard() // Убираем кнопку
            );
        }
    });
    // 3. ГОЛОС
